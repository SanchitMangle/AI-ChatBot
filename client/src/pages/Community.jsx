          ...
          <div className='flex flex-wrap max-sm:justify-center gap-5'>
            {
              images.map((item, index) => (
                <a key={index} href={item.imageUrl} target='_blank' className='relative group block rounded-lg overflow-hidden border border-gray-300 dark:border-purple-700 shadow hover:shadow-md transition-shadow duration-300'>
                  ...
                  <p className='absolute bottom-0 right-0 text-xs bg-black/50  backdrop-blur text-white px-4 py-1 rounded-tl-xl opacity-0 group-hover:opacity-100 duration-300'>Created By {item.userName}</p>
                </a>
              ))
            }
          </div>
